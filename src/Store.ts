  ...
  switch (action.type) {
    case "Set_Chat_Title":
      return {
        ...state,
        chatTitle:
          typeof action.chatTitle === "undefined" ? true : action.chatTitle,
      };
    case "Set_Locale":
      return {
        ...state,
...
export const history: Reducer<HistoryState> = (
  state: HistoryState = {
    activities: [],
    clientActivityBase:
      Date.now().toString() + Math.random().toString().substr(1) + ".",
  ...
  switch (action.type) {
    case "Set_AdaptiveCardsHostConfig":
      return {
        ...
        hostConfig:
          action.payload &&
          (action.payload instanceof HostConfig
            ? action.payload
...
const speakSSMLEpic: Epic<ChatActions, ChatState> = (action$, store) =>
  action$
    ...
    .merge(
      action$
        .ofType("Speak_SSML")
        .map((_) => ({ type: "Listening_Stopping" } as ShellAction))
    );

const speakOnMessageReceivedEpic: Epic<ChatActions, ChatState> = (
  action$,
  store
) =>
  action$
...
const showUploadBasedOnInputHint: Epic<ChatActions, ChatState> = (
  action$,
  store
) =>
  action$.ofType("Receive_Message").map(
    (action) =>
      ({
        type: "Toggle_Upload_Button",
...
const disableInputBasedOnInputHint: Epic<ChatActions, ChatState> = (
  ...
  action$
    ...
    .filter(
      (action) =>
        (action.activity as Message) &&
        store.getState().format.disableInputWhenNotNeeded
    )
    .map(
      (action) =>
        ({
          type: "Toggle_Disable_Input",
          disableInput: action.activity.inputHint === "ignoringInput",
        } as FormatAction)
    );
...
const startListeningEpic: Epic<ChatActions, ChatState> = (action$, store) =>
  action$
    ...
    .do(async (action: ShellAction) => {
      ...
      const lastMessageActivity = [...activities]
        .reverse()
        .find((activity) => activity.type === "message");
      // TODO: Bump DirectLineJS version to support "listenFor" grammars
      const grammars: string[] =
        lastMessageActivity && (lastMessageActivity as any).listenFor;
      const onIntermediateResult = (srText: string) => {
        store.dispatch({
          type: "Update_Input",
          input: srText,
          source: "speech",
      ...
      const onFinalResult = (srText: string) => {
        ...
        store.dispatch(
          sendMessage(srText, store.getState().connection.user, locale)
        );
      };
      const onAudioStreamStart = () => {
        store.dispatch({ type: "Listening_Start" });
      };
      const onRecognitionFailed = () => {
        store.dispatch({ type: "Listening_Stopping" });
      };
...
const listeningSilenceTimeoutEpic: Epic<ChatActions, ChatState> = (
  action$,
  store
) => {
...
const retrySendMessageEpic: Epic<ChatActions, ChatState> = (action$) =>
  action$.ofType("Send_Message_Retry").map(
    (action) =>
      ({
...
const updateSelectedActivityEpic: Epic<ChatActions, ChatState> = (
  action$,
  store
) =>
  action$
